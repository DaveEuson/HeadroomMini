...
pyinstaller --onefile --name YoyuCompanion --noconsole companion.py
```

The binary lands in `dist/`:
- Windows → `dist/YoyuCompanion.exe`
- macOS   → `dist/YoyuCompanion` (wrap in a `.app` or ship as-is)
- Linux   → `dist/YoyuCompanion`
